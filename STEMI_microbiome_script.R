...
#Do pairwise testing for significant findings ? Dunn test only for numerical, wilcoxon per group?
Kruskal_df %>% filter(Significant == "Yes") -> Kruskal_df_sig
list(Kruskal_df_sig$Variable) -> Pairwise_list

#load library
library(FSA)

#perform Dunn's Test with Bonferroni correction for p-values
dunnTest(statin ~ Risk_group,
         data=Metadata_kruskal,
         method="bonferroni")

str(Metadata_kruskal)
Metadata_kruskal[sapply(Metadata_kruskal, is.character)] <- lapply(Metadata_kruskal[sapply(Metadata_kruskal, is.character)], as.factor)

pairwise.wilcox.test(Metadata_kruskal$statin, Metadata_kruskal$Risk_group, p.adjust.method="bonferroni") -> wilcox_df

pwc2 <- Metadata_kruskal %>% 
  wilcox_test(statin ~ Risk_group, p.adjust.method = "bonferroni")
pwc2

#testing STEMI vs rest
Metadata_kruskal %>% mutate(STEMI = ifelse(Risk_group=="STEMI", "Yes", "No")) -> Metadata_kruskal
Metadata_kruskal[sapply(Metadata_kruskal, is.character)] <- lapply(Metadata_kruskal[sapply(Metadata_kruskal, is.character)], as.factor)
Metadata_kruskal[sapply(Metadata_kruskal, is.factor)] <- lapply(Metadata_kruskal[sapply(Metadata_kruskal, is.factor)], as.numeric)


wilcox.test(Metadata_kruskal$sex, Metadata_kruskal$STEMI)

install.packages("chisq.posthoc.test")
library(chisq.posthoc.test)
table(Metadata_kruskal$Risk_group, Metadata_kruskal$statin) %>% chisq.posthoc.test(simulate.p.value = TRUE, method = "bonferroni")

#using Pearson's Chi-squared test
table(Metadata_kruskal$proton_pump_inhib, Metadata_kruskal$Risk_group) -> mmatrix

chisq.test(mmatrix, correct = FALSE)
chisq.test(mmatrix[,c(1,2)], correct = FALSE) #low-intermediate
chisq.test(mmatrix[,c(1,3)], correct = FALSE) #low-high
chisq.test(mmatrix[,c(1,4)], correct = FALSE) #low-STEMI
chisq.test(mmatrix[,c(2,3)], correct = FALSE) #intermediate-high
chisq.test(mmatrix[,c(2,4)], correct = FALSE) #intermediate-STEMI
chisq.test(mmatrix[,c(3,4)], correct = FALSE) #high-STEMI
